...
NPT_String
VdrMediaServer::videoToDidl(NPT_String filter, cChannel *channel)
{
...
//     NPT_LOG_INFO(NPT_String(channel->Name()) + ": " + objectId);
    ...
    if (ins.second) {
        cerr << "VdrMediaServer::videoToDidl() new channel: " << (char*)objectId << endl;
...
NPT_String
VdrMediaServer::audioToDidl(NPT_String filter, cChannel *channel)
{
    PLT_MediaItem* object = new PLT_MediaItem();
    NPT_String objectId;
    PLT_MediaItemResource resource;
...
//     NPT_LOG_INFO(NPT_String(channel->Name()) + ": " + objectId);
    ...
    ins = m_itemCache.insert(make_pair(objectId, object));
    if (ins.second) {
        cerr << "VdrMediaServer::audioToDidl() new channel: " << (char*)objectId << endl;
        object->m_Title = channel->Name();
        object->m_ObjectClass.type = "object.item";
