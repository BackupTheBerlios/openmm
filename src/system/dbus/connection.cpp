...
{
        debug_log("Connection::Private::init()");
	...
	disconn_filter = new Callback<Connection::Private, bool, const Message&>(
        ...
        debug_log("Connection::Private::init() dbus_connection_add_filter()");
	dbus_connection_add_filter(conn, message_filter_stub, &disconn_filter, NULL);

        debug_log("Connection::Private::init() dbus_connection_set_dispatch_status_function()");
	dbus_connection_set_dispatch_status_function(conn, dispatch_status_stub, this, 0);
        debug_log("Connection::Private::init() dbus_connection_set_exit_on_disconnect()");
