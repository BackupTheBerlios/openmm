...
bool String::startsWith(const char *prefix) 
{
	if (!prefix)
		return false;
	if (regionMatches(0, prefix, 0, strlen(prefix)) == 0)
			return true;
	return false;
}
...
bool String::endsWith(const char *suffix) 
{
	if (!suffix)
		return false;
	if (regionMatches(length()-strlen(suffix), suffix, 0, strlen(suffix)) == 0)
		return true;
	return false;
}
