	...
	if (m_Request.substr(0, 4) == "GET " && CmdGET(m_Request.substr(4))) {
		...
		case hjListing:
			...
			break;
                ...
                case hjTransferRec:
//                         m_RecStreamerControl = new cRecStreamerControl(m_RecFileName.c_str());
//                         m_RecStreamerControl->Attach();
//                         cReplayControl::SetRecording(NULL, NULL);
                        cControl::Shutdown();
//                         cReplayControl::SetRecording(m_RecFileName.c_str(), m_RecFileName.c_str());
                        cControl::Launch(new cRecStreamerControl(m_RecFileName.c_str()));
                        cControl::Attach();
...
void cConnectionHTTP::Flushed(void) 
{
	...
	case hjTransfer:
		...
		break;
        case hjTransferRec:
		Dprintf("recstreamer start\n");
		m_RecStreamerControl->Play();
// 		m_RecStreamerControl->Activate(true);

                m_RecStreamerControl->Start(this);
		m_Status = hsFinished;
                break;
	}
}
...
bool cConnectionHTTP::CmdGET(const std::string &Opts) 
{
	...
	if (iterator) {
		...
		Dprintf("Apid is %d\n", apid);
		m_Job = hjTransfer;
        } else if (filespec.substr(filespec.length() - 4) == ".rec") {
                m_RecFileName = type + "/" + filespec + fileext;
                m_Job = hjTransferRec;
                Dprintf("Transfer Recording, RecFileName: %s\n", m_RecFileName.c_str());
	} else
		return false;
