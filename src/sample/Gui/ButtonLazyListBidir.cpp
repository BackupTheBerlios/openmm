...
{
    if (forward && _lastRowFetched < totalItemCount()) {
        int diff = std::min(rowCount, totalItemCount() - _lastRowFetched);
        _lastRowFetched += diff;
        return diff;
    }
    else if (!forward && _firstRowFetched >= 0) {
        int diff = std::min(rowCount, _firstRowFetched);
        _firstRowFetched -= diff;
        return diff;
...
{
    return forward ? _lastRowFetched : _firstRowFetched;
}
