...
int webserver_register_file(const char *path, const char *content_type)
{
	...
	snprintf(local_fname, PATH_MAX, "%s%s", "/usr/local/share/gmediarender",
	         strrchr(path, '/'));
...
int webserver_get_info(const char *filename, struct File_Info *info)
{
	...
	while (virtfile != NULL) {
	       printf("Searching for %s, comparing with %s\n", filename, virtfile->virtual_fname);
		if (strcmp(filename, virtfile->virtual_fname) == 0) {
                    printf("Found matching filename. len: %i\n", virtfile->len);
...
out:
	LEAVE();
	printf("webserver_get_info() returns: %i\n", result);
	return result;
}
